			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item">
					...
					<el-date-picker
						...
						type="daterange"
						range-separator="至"
						start-placeholder="计划日期起始"
						end-placeholder="计划日期结束"
				...
				<el-button class="list-add-btn" v-if="btnAuth('yinshijihua','新增')" type="primary" @click="add('/index/yinshijihuaAdd')">
			...
			<div class="list">
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)">
						...
						<div class="infoBox">
							<div class="name">{{item.jihuamingcheng}}</div>
							<div class="name">计划日期:{{item.jihuariqi}}</div>
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.timeRange.length > 0) {
					searchWhere.jihuariqistart = this.timeRange[0];
					searchWhere.jihuariqiend = this.timeRange[1];
				}
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`yinshijihua/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/yinshijihuaDetail', query: params});
			},
